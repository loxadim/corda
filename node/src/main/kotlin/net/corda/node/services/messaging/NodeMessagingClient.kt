...
class NodeMessagingClient(override val config: NodeConfiguration,
    ...
    private val version = SimpleString(version.toString())
    ...
    private fun artemisToCordaMessage(message: ClientMessage): ReceivedMessage? {
        try {
            if (!message.containsProperty(topicProperty)) {
                log.warn("Received message without a $topicProperty property, ignoring")
                return null
            }
            if (!message.containsProperty(sessionIdProperty)) {
                log.warn("Received message without a $sessionIdProperty property, ignoring")
                return null
            }
            val topic = message.getStringProperty(topicProperty)
            val sessionID = message.getLongProperty(sessionIdProperty)
